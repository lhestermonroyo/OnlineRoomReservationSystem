...
<div class="container">
  ...
  <hr>
      ...
      ?>
  <?php
    $cnt_res_query = $conn->query("select count(res_id) as res_id from reservation_tbl where cos_id = '".$cos_row['cos_id']."'");
    $cnt_res_row = $cnt_res_query->fetch();

    if ($cnt_res_row['res_id'] != 0) {
      $res_query = $conn->query("select * from reservation_tbl inner join room_tbl on reservation_tbl.room_id=room_tbl.room_id where cos_id = '".$cos_row['cos_id']."'");
      while ($res_row = $res_query->fetch()) {
        ...
        <div class="panel panel-default">
          <div class="panel-body">
            ...
            <?php
              include "modals/modal-cancelrequest-2.php";
            ?>
            <h3 class="margin-top-sm"><?=$res_row['room_name'];?> <span class="label label-warning"><?=$res_row['res_status'];?></span></h3>
            <p class="small text-muted margin-top-sm"><i class="fa fa-user fa-fw"></i> <?=$cos_row['firstname'];?> <?=$cos_row['lastname'];?> &bull; <i class="fa fa-clock fa-fw"></i> <?=date("F j, Y - h:ia", strtotime($res_row['res_added']));?></p>
